...
"""=============================START Maturity Caching========================================"""

@register_cache_policy('M_Cache')
class M_Cache(Cache):
    ...
    def get(self, k, *args, **kwargs):    
        if self.has(k):            
            ...
            self._cache[k] = freq + 1,t 
            #print("New Time :",t)
            #print("self._cache[k] :",self._cache)
            #print("\n\n\n")
            return True
        else:
            return False
    ...
    @inheritdoc(Cache)
    def put(self, k, node,time,*args, **kwargs):
        
        #print("Time:", time)
        #print("\n\n")        
        if not self.has(k):         
		  if(time < (100000+self.TIn)):
		    #self.t += 1
		    self.Time = time;
        	    #print("self._cache[k] :",self._cache)
                    #print("\n\n")
		    self._cache[k] = (1,time)  
		    if (len(self._cache) > self._maxlen):		
			evicted = min(self._cache, key=lambda x: self._cache[x])
			self._cache.pop(evicted)
			return evicted
		  else:                  
		      #evicted1 = min(self._cache, key=lambda x: self._cache[x])
		      #self._cache.pop(evicted1)
		      #print("Evicted :",evicted1)
		      #print("\n\n\n")
		      for key, v in self._cache.items():
		         self._cache[key]= (0,0)                  
		      self.TIn=time
		      self.t =0		  
		      #return evicted1   
...
"""=============================END Maturity Caching========================================"""




"""=============================Start Dynamic Windows ========================================"""

@register_cache_policy('IMU_DW')
class IMU_DW(Cache):
    """
    Dynamic-Window Movement (IMU-DW) cache replacement policy.
    Combines:
      1. Maturity classification
      2. Dynamic Window Adjustment
      3. IMU Replacement Policy
    """

    @inheritdoc(Cache)
    def __init__(self, maxlen, W_min=1, W_max=100, *args, **kwargs):
        self._cache = {}             
        self.eviction_freqs = {}     
        self.arrival_times = {}      
        self.freqs = {}              
    ...
    @inheritdoc(Cache)
    def maxlen(self):
        return self.W_t  # dynamic window size
    ...
    def get(self, k, *args, **kwargs):
        if self.has(k):
            ...
            self._cache[k] = (freq + 1, self.Time)
            self.freqs[k] = freq + 1
            self.arrival_times[k] = self.Time
            return True
        return False

    @inheritdoc(Cache)
    def put(self, k, node=None, time=0.0, *args, **kwargs):
        ...
        maturity_index = {}
        for s, (freq, t) in self._cache.items():
            T_p = self.W_t - self.arrival_times.get(s, self.Time)
            if T_p <= 0:
                T_p = 1
            maturity_index[s] = self.freqs.get(s, 1) / T_p

        median_maturity = 0
        if maturity_index:
            values = sorted(maturity_index.values())
            m = len(values)
            median_maturity = values[m // 2] if m % 2 else (values[m // 2 - 1] + values[m // 2]) / 2.0

        # ---------------- IMU-DW Window Update ----------------
        n = len(self.eviction_freqs) if self.eviction_freqs else 1
        total_evictions = sum(self.eviction_freqs.values())
        X = ((n - total_evictions) / n) * 100

        if X >= 50:
            W_t1 = self.W_t + self.W_t * (0.5 - (total_evictions / n))
            if W_t1 > self.W_max:
                W_t1 = self.W_max
        elif X < 50 and self.W_t > self.W_min:
            W_t1 = self.W_t - self.W_t * ((total_evictions / n) - 0.5)
        else:
            W_t1 = self.W_t
        self.W_t = max(self.W_min, min(int(W_t1), self.W_max))

        # ---------------- IMU Replacement Policy ----------------
        if self.window_expiry is None or self.Time < self.window_expiry:
            if k in self._cache:
                # Cache hit
                freq, _ = self._cache[k]
                self._cache[k] = (freq + 1, self.Time)
                self.freqs[k] = freq + 1
                self.arrival_times[k] = self.Time
            else:
                if len(self._cache) >= self.W_t:
                    # Evict least mature
                    victim = min(self._cache.keys(), key=lambda s: maturity_index.get(s, 0))
                    self.remove(victim)
                    self.eviction_freqs[victim] = self.eviction_freqs.get(victim, 0) + 1
                    evicted = victim
                # Insert new
                self._cache[k] = (1, self.Time)
                self.freqs[k] = 1
                self.arrival_times[k] = self.Time
        else:
            # Window expired: reset frequencies
            for s in self.freqs:
                self.freqs[s] = 1
            self.window_expiry = self.Time + self.W_t
            return self.put(k, node, time, *args, **kwargs)
    ...
    def remove(self, k, *args, **kwargs):
        if k in self._cache:
            self._cache.pop(k)
            self.freqs.pop(k, None)
            self.arrival_times.pop(k, None)
            return True
        return False
...
class PerfectLfuCache(Cache):
    ...
    def get(self, k, *args, **kwargs):
        ...
        if k in self._counter:
            freq, t, ti = self._counter[k]
            ti=self.Time 
            self._counter[k] = freq + 1, t,ti
        else:
            self._counter[k] = 1, self.t,self.Time
        if self.has(k):
            return True
        else:
            return False
        ...
        if not self.has(k):
          ...
          if(time < (100000+self.TIn)):
            if k in self._counter:
                ...
                self._counter[k] = (freq + 1, t,ti)
                self.Time = time;
                #print("self._counter :",self._counter)
		#print("\n\n\n")
            else:
                # If I always call a get before a put, this line should never
                # be executed
                self._counter[k] = (1, self.t,time)
            self._cache.add(k)
            if len(self._cache) > self._maxlen:
                evicted = min(self._cache, key=lambda x: self._counter[x])
                self._cache.remove(evicted)
                self._counter.pop(evicted)
                return evicted
          else:                  
		for key,v in self._counter.items():
		  self._counter[key]= (0,0,0)                  
		self.TIn=time
